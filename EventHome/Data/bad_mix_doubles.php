...
<html lang="en">
	<head>
		<title> CO-Olympics | Badminton-Mix Double Data </title>
		<meta charset="utf-8">
			...
			<div class="container-fluid">
				<div class="container col-md-2 col-lg-2 bor" style="background-color:#0d1a26;border-color: white;border-radius: 2px;">
					...
					<ul class="nav nav-pills nav-stacked" id="el" style="text-align:left">
						...
						<li><a style="color: white;font-size: 20px;" href="football.php">Football</a></li>
						<li class="active"><a style="color: white;font-size: 20px;" href="badminton.html">Badminton</a></li>
				...
				<div>
                    <?php   
                        ...
                        $stmt = $con->prepare("SELECT * FROM bad_mix_doubles");
                        $stmt->execute();
                        $rows = $stmt->fetchAll();

                        echo "<h1 style=\"font-family:zcool xiaowei;color:white;\"> Badminton (Mix Doubles) </h1>  <br><br>";
                        echo "<table>";
                        echo "<tr> <th> ID </th> <th> Player 1 </th> <th> Player 2 </th> <th> Contact No. </th> <th> Year </th> <th> Shift </th> </tr>"; 

                        if(count($rows) > 0)
                        {
                            foreach($rows as $row)
                            {
                                echo "<tr> <td> ".$row['id']." </td> <td> ".$row['p1']." </td> <td> ".$row['p2']." </td> <td> ".$row['contact']." </td> <td> ".$row['year']." </td> <td> ".$row['shift']."</td>";
                            }
                        }
