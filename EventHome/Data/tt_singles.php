...
<html lang="en">
	<head>
		<title> CO-Olympics | Table Tennis-Singles Data </title>
		<meta charset="utf-8">
			...
			<div class="container-fluid">
				...
				<div>
                    <?php   
                        ...
                        $stmt = $con->prepare("SELECT * FROM tt_singles");
                        $stmt->execute();
                        $rows = $stmt->fetchAll();

                        echo "<h1 style=\"font-family:zcool xiaowei;color:white;\"> Table Tennis (Singles) </h1>  <br><br>";
                        echo "<table>";
                        echo "<tr> <th> ID </th> <th> Name </th> <th> Contact No. </th> <th> Year </th> <th> Shift </th> </tr>"; 

                        if(count($rows) > 0)
                        {
                            foreach($rows as $row)
                            {
                                echo "<tr> <td> ".$row['id']." </td> <td> ".$row['p1']." </td> <td> ".$row['contact']." </td> <td> ".$row['year']." </td> <td> ".$row['shift']."</td>";
                            }
                        }
